...
class TestCommentLikesView(TestCase):
    ...
    def test_get_post_likes(self):
        ...
        author_liker = create_author()
        author_liker_json = create_author_dict(author_liker.id)
        create_like(author_liker_json, self.post, None)
        create_like(author_liker_json, self.post, self.comment)

        url = reverse("liked", args=[author_liker.id])
        likes_response = self.client.get(url, **self.headers)
        likes_json_obj = deserialize_response(likes_response)

        self.assertEqual(likes_response.status_code, status.HTTP_200_OK)
        self.assertEqual(likes_json_obj["type"], "liked")
        self.assertEqual(len(likes_json_obj["items"]), 2)
        self.assertEqual(likes_json_obj["items"][0]["object"], post_json_obj["id"])
        self.assertEqual(likes_json_obj["items"][0]["summary"],
                         f"{author_liker_json['displayName']} likes your post")
        self.assertEqual(likes_json_obj["items"][1]["object"], comment_url)
        self.assertEqual(likes_json_obj["items"][1]["summary"],
                         f"{author_liker_json['displayName']} likes your comment")
